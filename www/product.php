...
<?php include "connect.php"; ?>
  ...
  <div class="content-wrapper">
    ...
    <form action="<?php echo $_SERVER['SCRPIT_NAME']?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
      <table class="table table-bordered" cellpadding="1" cellspacing="1">
      <tr>
          <td colspan="2" align="center"><h4>เพิ่มสินค้า</h4></td>    
      </tr>
      <tr>
          <td width="20%"><label>รหัสสินค้า</label></td>
          <td width="80%"><label for="id"></label>
          <input class="form-control" type="text" name="id" id="id" placeholder="กรอกรหัสสินค้า"/></td>
      </tr>
      <tr>
          <td width="20%"><label>ชื่อสินค้า</label></td>
          <td width="80%"><label for="name"></label>
          <input class="form-control" type="text" name="name" id="name" placeholder="กรอกชื่อสินค้า"/></td>
      </tr>
      <tr>
      <td width="20%"><label>ประเภทสินค้า</label></td>
          <td width="80%"><label for="type"></label>
          <select name='type' id="type" class='form-control type'>
                                             ...
                                             <option value="MORNITOR">MONITOR</option>
		        </select>
          </td>
      </tr>
      <tr>
      <td width="20%"><label>รายละเอียดสินค้า</label></td>
          <td width="80%"><label for="name"></label>
          <textarea rows="3" cols="60" id="desc" name="desc" class="form-control text-con" placeholder="กรอกรายละเอียดสินค้า"><?php echo $row["pro_desc"];?></textarea></td>
      </tr>
      <tr>
      <td width="20%"><label>ราคาสินค้า</label></td>
          <td width="80%"><label for="type"></label>
          <input class="form-control" type="text" name="price" id="price" placeholder="กรอกราคาสินค้า"/></td>
      </tr>
      <tr>
      <td width="20%"><label>จำนวนสินค้า</label></td>
          <td width="80%"><label for="type"></label>
          <input class="form-control" type="text" name="amount" id="amount" placeholder="กรอกจำนวนสินค้า"/></td>
      </tr>
      <tr>  
		  ...
		  <td width="70%">
            ...
            <br/>
            <input type="file" name="img" id="img" value="" onchange="readURL(this);" />
             <input name="file1" type="hidden" id="file1" value="none" />
          </td>
	    </tr>
      <tr>
          <td>&nbsp;</td>
          <td><input class="btn btn-success" type="submit" name="save" id="save" value="บันทึก" /></td>
      </tr>
      </table>
    ...
    <?php
      if(isset($_POST['save']) && $_POST["id"]!="" && $_POST["name"]!="" && $_POST["type"]!="") {
	...
	move_uploaded_file($_FILES['img']['tmp_name'],$path_copy);  
		...
		$path_copy = $file1;//ถ้าไม่เลือกรูปให้กลับเป็นรูปเดิม
		}

        
        $sql = "INSERT INTO product (pro_id,pro_type,pro_name,pro_desc,pro_price,pro_amount,pro_img,addpro_date) VALUES 
                ('".$_POST["id"]."','".$_POST["type"]."','".$_POST["name"]."','".$_POST["desc"]."','".$_POST["price"]."','".$_POST["amount"]."','$path_copy',now())";
          ...
          <div class="card">
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  <td><img src='<?php echo $row["pro_img"]; ?>' width="50" height="40" /></td>
                  ...
                  <td>
                    <div class="btn-group">
                        <button type="button" name="edit" id="<?php echo $row["pro_id"]; ?>" class="btn btn-info view_data"><i class="fas ion-md-create col1"></i></button>
                        <form method="post" action="delete.php">
                        <input type="hidden" name="pro_id" value="<?php echo $row['pro_id']; ?>">  
                        <button type="submit" name="delete" onclick="return confirm('คุณต้องการลบข้อมูลที่เลือก')" class="btn btn-danger del_data"><i class="fas ion-md-trash col1"></i></button>
                        </form>
                    </div>
                </td>
                </tr>

  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">การคืนสินค้า</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
      ...
      <div class="row">
        <div class="col-12">
            ...
            <div>เลือกช่วงเวลาที่ต้องการ</div> 
                <!-- Date range -->
                <form method="post" action="printReturn.php">
          ...
          <div class="container-fluid">
             <div align="center"><h4>ร้านขายอุปกรณ์คอมพิวเตอร์ Computer-Store</h4></div>
              ...
              <tr>
              <td>
              <strong>รายงานสรุปการคืนสินค้า <?php if($_POST['date'] != ""){ echo "ตั้งแต่วันที่ ".$_POST['date']; }?></strong>
              </td>
            ...
            <div class="card-body">
              <table id="example2" class="table table-bordered table-striped">
                ...
                if($_POST['date'] != ""){$result = mysqli_query($con,"SELECT * FROM return_pro WHERE return_date BETWEEN '$sdate' AND '$edate'");}else{
                $result = mysqli_query($con,"SELECT * FROM return_pro");}
                ...
                <tr>
                  ...
                  <td>
                  <?php $proid = $row['pro_id']; 
                  $result4 = mysqli_query($con,"SELECT * FROM product WHERE pro_id = '$proid'"); 
                  $row4 = mysqli_fetch_assoc($result4); echo $sumprice = intval($row4['pro_price']) * intval($row['return_amount']); ?>
                  </td>
                  <td><?php $time=$row['return_date']; echo Thai_date($time); ?></td>
                </tr>
                </form>
                <?php $count = $count + 1; $allamount = $allamount + intval($row['return_amount']); $allprice = $allprice + $sumprice; } mysqli_close($con);?>
                
                </tbody>

                <tfoot>
                <tr>
                  <th>รายการคืนสินค้า</th>

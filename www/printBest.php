  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">สินค้าขายดี</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
    ...
    <section class="content">
      <div class="row">
        ...
        <div>เลือกช่วงเวลาที่ต้องการ</div>
                <!-- Date range -->
                <form method="post" action="printBest.php">
          ...
          ?>
            ...
            <div class="card-body">
              <table id="" class="table table-bordered">
                ...
                if($_POST['date'] != "" && $type != ""){$result = mysqli_query($con,"SELECT * ,sum(pro_qty) as sum_qty FROM count_salepro WHERE count_date BETWEEN '$sdate' AND '$edate' and pro_type = '$type' group by pro_id order by sum_qty desc,pro_price desc LIMIT 10");}
                else if($type2 != ""){$result = mysqli_query($con,"SELECT * ,sum(pro_qty) as sum_qty FROM count_salepro WHERE pro_type = '$type2' group by pro_id order by sum_qty desc,pro_price desc LIMIT 10");}
                else if($_POST['date'] != ""){$result = mysqli_query($con,"SELECT * ,sum(pro_qty) as sum_qty FROM count_salepro WHERE count_date BETWEEN '$sdate' AND '$edate' group by pro_id order by sum_qty desc,pro_price desc LIMIT 10");}
                else {$result = mysqli_query($con,"SELECT * ,sum(pro_qty) as sum_qty FROM count_salepro group by pro_id order by sum_qty desc,pro_price desc LIMIT 10");}
                ...
                <tr>
                  ...
                  <td><?php $sumprice = intval($row['pro_price']) * intval($row['sum_qty']) ; echo number_format($sumprice) ?></td>
                </tr>
                </form>
